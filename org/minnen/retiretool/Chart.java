...
public class Chart
{
  ...
  public final static int[][]  colorsRGB = { { 132, 111, 220 }, { 133, 218, 209 }, { 201, 165, 57 }, { 40, 63, 52 },
      { 211, 78, 160 }, { 209, 164, 210 }, { 193, 213, 147 }, { 70, 93, 130 }, { 94, 153, 57 }, { 198, 151, 107 },
      { 93, 144, 119 }, { 201, 218, 75 }, { 205, 84, 213 }, { 108, 220, 150 }, { 218, 70, 54 }, { 221, 127, 48 },
      { 105, 140, 213 }, { 217, 143, 151 }, { 62, 38, 68 }, { 139, 56, 103 }, { 89, 105, 45 }, { 111, 70, 145 },
      { 85, 33, 33 }, { 136, 111, 121 }, { 146, 77, 38 }, { 139, 181, 204 }, { 221, 68, 112 }, { 83, 66, 32 },
      { 165, 64, 71 }, { 105, 219, 73 } };

  public final static String[] colorsHex = { "#846FDC", "#85DAD1", "#C9A539", "#283F34", "#D34EA0", "#D1A4D2",
      "#C1D593", "#465D82", "#5E9939", "#C6976B", "#5D9077", "#C9DA4B", "#CD54D5", "#6CDC96", "#DA4636", "#DD7F30",
      "#698CD5", "#D98F97", "#3E2644", "#8B3867", "#59692D", "#6F4691", "#552121", "#886F79", "#924D26", "#8BB5CC",
      "#DD4470", "#534220", "#A54047", "#69DB49" };
  ...
  public static void saveHoldings(File file, Map<LocalDate, DiscreteDistribution> holdings) throws IOException
  {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(file))) {
      ...
      writer.write(String.format("</head><body style=\"width:%dpx\">\n", width));
      writer.write("<table id=\"holdingsTable\" style=\"width:100%\">\n");
      writer.write("<tbody>\n");
      ...
      for (Map.Entry<LocalDate, DiscreteDistribution> entry : holdingsReversed) {
        ...
        DiscreteDistribution dist = entry.getValue();
        writer.write(String.format("<tr><td style=\"width:10%%\">%s</td><td style=\"width:90%%\">", date));
        for (int i = 0; i < dist.size(); ++i) {
          if (dist.weights[i] < 0.009) continue;
          String symbol = dist.names[i];
          if (!symbol2color.containsKey(symbol)) {
            symbol2color.put(symbol, colorsHex[colorIndex]);
            colorIndex = (colorIndex + 1) % colorsHex.length;
          }
          writer.write(String.format(
              "<span style=\"float: left; width: %.2f%%; background: %s; white-space: nowrap;\">%s (%.1f)</span>\n",
              dist.weights[i] * 90, symbol2color.get(symbol), symbol, dist.weights[i] * 100));
        }
        writer.write("</td></tr>");
      }
      writer.write("</tbody>\n</table>\n");
