...
public class TestCumulativeStats
{
  ...
  {
    double[] values = new double[] { 1, 2, 3, 1, 0, 2, 3, 3, 2 };
    Sequence daily = new Sequence("test");
    LocalDate date = LocalDate.of(2000, Month.JANUARY, 10);
    for (int i = 0; i < values.length; ++i) {
      daily.addData(values[i], date);
      if (i % 2 == 0) date = date.plusDays(10);
      else date = date.minusDays(10).plusMonths(1);
    }

    // Validate stats for daily data.
    CumulativeStats stats = CumulativeStats.calc(daily);
    assertEquals(700.0, stats.cagr, 1e-6);
    assertEquals(100.0, stats.drawdown, 1e-6);
    assertEquals(2.0, stats.totalReturn, 1e-6);

    // Validate stats for monthly data.
    Sequence monthly = FinLib.dailyToMonthly(daily, 0, 0, 2);
    stats = CumulativeStats.calc(monthly);
    assertEquals(406.25, stats.cagr, 1e-6);
    assertEquals(50.0, stats.drawdown, 1e-6);
    assertEquals(1.5, stats.totalReturn, 1e-6);
  }
}
